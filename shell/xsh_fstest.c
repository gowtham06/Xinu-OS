...
shellcmd xsh_fstest(int nargs, char *args[]) {
    ...
    buf3 = getmem(1000*sizeof(char)); 
    buf4 = getmem(1000*sizeof(char));
...
/*-----------------------------------Mycode for testing 2nd file------------------------------------*/
fd = fs_create("Rohan", O_CREAT);

    strcpy(buf3,"aaaaaaaabbbbbbbbbcccccccccdddddddeeeeeeefffffffffjjjjjjjjjjssssssssssskkkkkkkaaaaaaaaallllllleeeeeee;;;;;;;;1111112333334444444445555677777777788888890000---------------=======555555000");
    rval = fs_write(fd,buf3,strlen(buf3));

  if(rval == 0 || rval != strlen(buf3) )
    {
     	printf("\n\r File write failed");
        goto clean_up;
    }
